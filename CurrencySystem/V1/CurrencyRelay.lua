...
-- SUPPORTS MULTIPLE LINKED CARDS
-- Receives from clients via TUNNEL (linked cards)
-- Forwards to server via WIRELESS
...
-- Configuration
local PORT = 1000
local RELAY_NAME = "Multi-Client Relay"
...
local function updateDisplay()
    ...
    print("═══════════════════════════════════════════════════════")
    print("")
    print("Mode: MULTI-TUNNEL ←→ WIRELESS")
...
local function handleMessage(eventType, _, sender, port, distance, message)
    ...
    if isTunnel then
        ...
        -- Forward to server via wireless
        addToLog("→ SERVER: Forwarding (" .. #message .. " bytes)", "SERVER")
        
        local sendOk, sendErr = pcall(modem.send, serverAddress, PORT, message)
        if sendOk then
            stats.messagesForwarded = stats.messagesForwarded + 1
            addToLog("  Wireless send: SUCCESS", "SUCCESS")
        ...
        if sender == serverAddress then
            ...
            if targetTunnel then
                ...
                local sendOk, sendErr = pcall(targetTunnel.send, message)
                if sendOk then
...
-- Main
local function main()
    print("Starting Multi-Client Currency Relay...")
    ...
    addToLog("Multi-Client relay started (" .. #tunnels .. " cards)", "SUCCESS")
